    ...
    uploadXml(ev: Event) {
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        if (event.target) {
          ...
          */
          const _lf = this.$props.lf
          _lf.renderRawData(JSON.parse(data));
        }
      };
      reader.readAsText(file); // you could also read images and other binaries
    },
    getGraphRawData() {
      const data = (this.$props.lf as LogicFlow).getGraphRawData();
      this.jsonData = data;
      this.showDrawer = true;
      download("logic-flow.txt", JSON.stringify(data));
    },
  },
};
</script>
